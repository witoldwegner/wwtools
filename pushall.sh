...
for branch in $branches; do
    ...
    original_remote=$(git config branch."$branch".remote)
    ...
    # Set the branch back to the original remote
    echo "git branch --set-upstream-to=\"$original_remote/$branch\" \"$branch\""
    git branch --set-upstream-to="$original_remote/$branch" "$branch"
done
